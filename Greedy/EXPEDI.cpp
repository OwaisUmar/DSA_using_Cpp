...
int main() {
    ...
    while(t--) {
        ...
        for(int i=0; i<N; i++) {
            cin>>a[i].first;        //Distance from the town to the stop
            cin>>a[i].second;       //amount of fuel available at that stop
        }
        int L, P;
        cin>>L>>P;
        for(int i=0; i<N; i++)
            a[i].first=L-a[i].first;      //Distance from the truck to the stop
